...
if choice == '0':
    ...
    x = ['мужчины', 'женщины']
    y = [f'{survived_men_1_2 / number_men_1_2 * 100:.2f}%', f'{survived_women_1_2 / number_women_1_2 * 100:.2f}%']
    tab = {'Пол пассажира': x, 'Доля выживших пассажиров': y}
    st.dataframe(tab)
    fig = plt.figure(figsize=(8, 3))
    plt.bar(x, [int(survived_men_1_2 / number_men_1_2 * 100), int(survived_women_1_2 / number_women_1_2 * 100)], color='brown')
    plt.xlabel('Пол пассажира')
    plt.ylabel('Доля выживших пассажиров, %')
    plt.title('Доля выживших пассажиров')
    st.pyplot(fig)
